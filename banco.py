...
def criarconta():
    ...
    saldo.append(0)
    numeroconta.append(input("Numero da conta"))
    limpar()
    menu() 
...
def depositar():
    ...
    if conta == numeroconta[i]:
         ...
         historico.append(f"Você recebeu um valor de: {valor},hora:{hora.strftime("%H:%M:%S")},na conta:{conta} ")
    inicio = input("Digite 'sair' para voltar para o início: ")
    if inicio == 'sair':
        limpar()
        menu()
def pix():
     ...
     if conta == numeroconta[i]:
         saldo[i]=saldo[i]-valor
         historico.append(f"Você fez um pix de:{valor},no dia:{data},hora:{hora.strftime("%H:%M:%S")},na conta:{conta} ")
     print("Pix realizado")
     outraconta=input("Digite a conta que você realizou o pix:")
     valor=input('Digite o valor que foi transferido:')
     encontrado=False
     for i in range(len(numeroconta)):
            if numeroconta[i] == outraconta:
                print(f"Conta: {numeroconta[i]}\nCPF: {cpf[i]}\nNome: {nome[i]}\nSaldo: {saldo[i]}")
                encontrado = True
                break
